...
class HomebaseController extends AppframeController {
	...
	function _initialize() {
		...
		if(empty(session('header'))){
		    //产品分类
		    $cates=M('Cate')->field('id,name')->where('fid=0')->order('sort desc,id asc')->limit('0,5')->select();
		    session('header.cate',$cates);
		    session('header.cateCount',(count($cates)==5)?1:0);
		    //企业模板分类
		    $tcates=M('Tcate')->field('id,name')->where('fid=0')->order('sort desc,id asc')->limit('0,5')->select();
		    session('header.tcate',$tcates);
		    session('header.tcateCount',(count($tcates)==5)?1:0);
		    //案例行业分类
		    $ccates=M('Ccate')->field('id,name')->where('fid=0')->order('sort desc,id asc')->limit('0,5')->select();
		    session('header.ccate',$ccates);
		    session('header.ccateCount',(count($ccates)==5)?1:0);
		    //报价分类
		    $ocates=M('Ocate')->field('id,name')->where('fid=0')->order('sort desc,id asc')->limit('0,5')->select();
		    session('header.ocate',$ocates);
		    session('header.ocateCount',(count($ocates)==5)?1:0);
		    //新闻分类
		    $ncates=M('Ncate')->field('id,name')->where('fid=0')->order('sort desc,id asc')->limit('0,5')->select();
		    session('header.ncate',$ncates);
		    session('header.ncateCount',(count($ncates)==5)?1:0);
		    
		    //关于
		    $acates=M('Acate')->field('id,name')->where('fid=0')->order('sort desc,id asc')->limit('0,5')->select();
		    session('header.acate',$acates);
		    session('header.acateCount',(count($acates)==5)?1:0);
		    
		    //读取网站配置
		    $tmp=M('Company')->order('name asc')->select();
		    $company=array();
		    foreach($tmp as $v){
		        if($v['type']==1){
		            $company[$v['name']][]=$v;
		...
		$this->assign("header0",session('header'));
